...
class Player(pygame.sprite.Sprite):
    ...
    def move(self, dx, dy, dz, locationMap):
        if (self.getLegalPlayerPos(dx, dy, dz, locationMap) == None):
            print("Cannot move to:", (self.gameX + dx, self.gameY + dy, self.gameZ + dz))
    ...
    def placeBlock(self, block, locationMap, gameBlockGroup):
        if (self.getPlaceBlockLocation(locationMap) == None):
    ...
    def getPlaceBlockLocation(self, locationMap):  # gets location at which block has to be placed
        if (self.direction == "+y"): # facing down the y direction
            (x, y, z) = (self.gameX, self.gameY + 1, self.gameZ + 1)  # coords being considered; add 1 to gameZ beause
    ...
    def getLegalPlayerPos(self, dx, dy, dz, locationMap):  # gets location at which block has to be placed
        if (dx != 0): # move in x
            ...
            if (locationMap[x, y, z].name == "empty" and locationMap[x, y, z + 1] == "empty"): # player takes up two z-blocks
                return (x, y, z)
            else: return False
        elif (dy != 0): # move in y
...
# Isometric drawing VERY HEAVILY adapted from http://clintbellanger.net/articles/isometric_math/
    ...
    def drawBlock(self, screen, block, centerPos, playerPos):
        ...
        drawX = ((x - posX) - (y - posY))*Minecraft.blockXWidth/2 + self.width/2 + Minecraft.blockXWidth/2 - Minecraft.blockXWidth  #new test
